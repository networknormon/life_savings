...
// --- CONTROLES DE LA INTERFAZ (MENÚ Y PANELES) ---
window.toggleMenu = () => {
...
// AHORA USA LA CLASE .open PARA ANIMAR CON CSS GRID
...
if (isShowcase) {
    ...
    document.getElementById('hamburger-btn').style.display = 'none'; // Ocultamos el menú lateral al visitante
    document.getElementById('header-badges').innerHTML = '<span class="badge success" style="font-size:1rem;">👁️ Vitrina Pública</span>';
...
// Inyectar botones dinámicos en el MENU LATERAL
document.addEventListener('DOMContentLoaded', () => {
    ...
    scryfallBtn.style.padding = '0.8rem';
    scryfallBtn.style.borderColor = '#8b5cf6';
    scryfallBtn.style.color = '#8b5cf6';
    scryfallBtn.style.textAlign = 'left';
    scryfallBtn.innerHTML = '🔄 Precios Magic';
    scryfallBtn.onclick = () => { window.syncScryfallPrices(); window.toggleMenu(); };
    
    const summaryBtn = document.createElement('button');
    summaryBtn.className = 'btn';
    summaryBtn.style.padding = '0.8rem';
    summaryBtn.style.borderColor = 'var(--primary)';
    summaryBtn.style.color = 'var(--primary)';
    summaryBtn.style.textAlign = 'left';
    summaryBtn.innerHTML = '📊 Resumen Anual';
    summaryBtn.onclick = () => { window.showAnnualSummary(); window.toggleMenu(); };
    
    // Insertamos los botones justo encima del de "Cerrar sesión"
    const logoutBtn = sideMenu.lastElementChild;
    sideMenu.insertBefore(scryfallBtn, logoutBtn);
    sideMenu.insertBefore(summaryBtn, logoutBtn);
});
